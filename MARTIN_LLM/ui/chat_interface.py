...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QRadioButton, QSpacerItem,
                             ...
                             QListWidgetItem, QFileDialog, QApplication, QSizePolicy, QToolButton)
...
class ChatInterface(QMainWindow):
    ...
    def __init__(self, user_id, username, chat_engine: ChatEngine, user_service: UserService, parent=None):
        ...
        self.add_system_message("SISTEMA INICIALIZADO Y LISTO.")
    ...
    def create_central_area(self, parent_layout):
        ...
        chat_layout.addWidget(self.history_text, stretch=1)
    ...
    def create_mode_selection_widget(self):
        ...
        self.toggle_panel_button = QPushButton(qta.icon('fa5s.align-justify'), "")
    ...
    def populate_recent_conversations(self):
        ...
        try:
            ...
            for conv in sorted_convs:
                ...
                item_text = f"[{date_str}] {title}" # Mostrar título completo
                ...
                label.setMinimumHeight(30) # Aumentar altura para mejor centrado vertical
                label.setToolTip(f"Doble clic para cargar: {title}")

                delete_button = QPushButton(qta.icon('fa5s.trash-alt'), "")
                delete_button.setFixedSize(24, 24)
                delete_button.setToolTip("Ocultar esta conversación de la lista")
                delete_button.setStyleSheet("QPushButton { border: none; background: transparent; padding: 0; min-width: 0; } QPushButton:hover { background-color: #e53e3e; border-radius: 4px; }")
                ...
                size_hint.setHeight(max(size_hint.height(), 40)) # Aumentar altura de la fila
    ...
    def on_model_selected(self, model_identifier):
        ...
        try:
            ...
            self.add_to_history("CHAT-AI", "Sistema listo para recibir comandos.")
    ...
    def add_system_message(self, message):
        """Agregar mensaje del sistema con estilo especial"""
        cursor = self.history_text.textCursor()
        cursor.movePosition(QTextCursor.MoveOperation.End)
        # Formato para mensajes del sistema
        format_system = QTextCharFormat()
        format_system.setForeground(QColor("#68c964")) # Color rosa/rojo del tema
        format_system.setFontWeight(QFont.Weight.Bold)
        cursor.insertText(f"[SISTEMA] {message}\n", format_system)
        # Scroll automático
    ...
    def add_to_history(self, role, message):
        ...
        # Formato según el rol
        if role == "Usuario":
            color = QColor("#366de4") # Color azul del tema
            prefix = f"[{timestamp}] USER@TERMINAL:\n"
        else:
            color = QColor("#366de4") # Color verde del tema
            prefix = f"[{timestamp}] CHAT-AI:\n"
        
        # Formato para el header
        format_header = QTextCharFormat()
        format_header.setForeground(color)
        format_header.setFontWeight(QFont.Weight.Bold)
        
        # Formato para el mensaje
        format_message = QTextCharFormat()
        format_message.setForeground(QColor("#e1e5e9")) # Color de texto principal
        
        cursor.insertText(prefix, format_header)
    ...
    def send_message(self):
        ...
        self.add_to_history("Usuario", user_message)
        # Añadir el mensaje del usuario al historial lógico ANTES de llamar al worker
        if self.chat_engine:
            self.chat_engine.history.append({"role": "user", "content": user_message})
    ...
    def run_agent_worker(self, user_message: str):
        """Inicia el worker para ejecutar el agente."""
        ...
        self.add_system_message("AGENTE INICIADO. OBJETIVO: " + user_message)
    ...
    def handle_agent_response(self, response: str):
        ...
        self.add_system_message("AGENTE HA FINALIZADO LA TAREA.")
        # Añadir la respuesta final del agente al historial lógico
        if self.chat_engine:
            self.chat_engine.history.append({"role": "assistant", "content": response})
        self.handle_response(response) # Reutiliza el manejador de respuesta normal
    def handle_response(self, response):
        """Maneja la respuesta exitosa del worker."""
        print("[UI] Manejando respuesta del worker.")
        self.add_to_history("CHAT-AI", response)
        # Añadir la respuesta del asistente al historial lógico
        if self.chat_engine:
            self.chat_engine.history.append({"role": "assistant", "content": response})
        # Ahora, con el historial completo y actualizado, guardamos.
    ...
    def handle_error(self, error_msg):
        ...
        self.add_to_history("CHAT-AI", f"ERROR: {error_msg}")
        self.send_button.setEnabled(True)
        self.input_text.setFocus()
        ...
        try:
            if not self.chat_engine.conversation_id:
                ...
                if new_id:
                    ...
                    if not is_autosave:
                        self.add_system_message("CONVERSACIÓN GUARDADA EN LA BASE DE DATOS")
                ...
                if not is_autosave:
                    self.add_system_message("CONVERSACIÓN ACTUALIZADA EN LA BASE DE DATOS")
                    QMessageBox.information(self, "Actualizado", 
    ...
    def repopulate_history_ui(self):
        """Vuelve a dibujar el historial en la UI a partir de self.chat.history."""
        for msg in self.chat_engine.history:
            role = msg.get("role")
            content = msg.get("content")
            if role == "user":
                self.add_to_history("Usuario", content)
            elif role == "assistant":
                self.add_to_history("CHAT-AI", content)
    ...
    def on_mode_changed(self, button, checked):
        ...
        if button is self.chat_mode_radio:
            ...
            self.reasoner_mode = False
            self.add_system_message("MODO AGENTE ACTIVADO. Los prompts se tratarán como objetivos para el agente.")
            ...
            self.add_system_message("MODO RAZONADOR ACTIVADO. Los prompts se tratarán como objetivos complejos a planificar.")
