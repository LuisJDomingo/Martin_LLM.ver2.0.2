...
class FineTuningWidget(QFrame):
    ...
    def on_start_clicked(self):
        ...
        if not new_model_name or ":" not in new_model_name:
            QMessageBox.warning(self, "Nombre Inválido", "Por favor, especifica un nombre para el nuevo modelo, incluyendo una etiqueta (ej: 'mi-modelo:latest').")
            return

        if not training_data:
            QMessageBox.warning(self, "Datos Vacíos", "Por favor, proporciona los datos de entrenamiento en formato JSONL.")
            return
    ...
    def on_finetuning_finished(self, success: bool, message: str):
        ...
        if "cancelado" in message.lower():
            ...
            QMessageBox.information(self, "Éxito", f"Fine-tuning completado con éxito.\n{message}")
        else:
            QMessageBox.critical(self, "Error", f"El fine-tuning ha fallado.\n{message}")